...
"total_marks":          100,
"question_types":       ["Structured"],
"difficulty":           "Extremely Difficult only 2 out 10 student can solve",
...
"total_marks":          100,
"question_types":       ["Structured"],
"difficulty":           "Extremely Difficult only 1 out 20 student can solve",
"number_of_questions":  35,
"marks_per_question":   "2-4 depending on the question",
}
...
• Follow the format of Cambridge A Level Physics Paper 4 questions.
• Only use the content from the textbook provided above to generate all questions. Do not use prior knowledge.
• Include data-based, heory and experimental-evaluation questions.
• Make the question length atleast three lines without mentioning formula.
• Make the questions very analytical and complex where atleast 2 formulas needs to be used on numerical based questions.
• Include a comprehensive model \`"answer"\` for each question (concise but mark-worthy).
• Avoid multiple-choice format. Each question should be structured, and show mark allocations.
• Ensure output is strictly valid UTF-8 encoded JSON. No comments, no markdown, no trailing commas.
...
const getPromptTemplate = (board, classLevel, subject, paperType, difficulty = 'Medium') => {
  ...
  const paperConfig = paperPatterns[board][classLevel][subject][paperType];
  
  // If difficulty-specific template exists, use it, otherwise use the default template
  if (difficulty && paperConfig[difficulty] && paperConfig[difficulty].prompt_template) {
    return paperConfig[difficulty].prompt_template;
  } else if (paperConfig.prompt_template) {
    return paperConfig.prompt_template;
  }
